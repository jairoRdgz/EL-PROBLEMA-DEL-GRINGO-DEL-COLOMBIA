...
public class AdjMatrixGraphTest {
	...
	@Test
	protected void floydwarshallTest() {
		setupStage1();

		double[][] product = grafo.floydwarshall();

		double[][] asItShouldBe = new double[4][4];
		...
		for (int i = 0; i < product.length - 1; i++) {
			for (int j = 0; j < product.length - 1; j++) {
				assertTrue(asItShouldBe[i][j] == product[i][j], "La ruta minima entre " + i + " y " + j
						+ " debería ser " + asItShouldBe[i][j] + " pero es " + product[i][j]);
			}
		}
	}
